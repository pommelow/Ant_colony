...
void initializeFT(float* ptr_prev, float* ptr_next, float* ptr_vel, Parameters* p, size_t nbytes, int n1_Tblock, int n2_Tblock, int n3_Tblock, int nThreads){
...
#  error "HALF_LENGTH not implemented"
	...
	for(int i=1; i<= HALF_LENGTH; i++){
  	...
  	printf("allocating prev, next and vel: total %g Mbytes\n",(3.0*(nbytes+16))/(1024*1024));fflush(NULL);
        ...
        hbw_posix_memalign((void**)(&(prev_base)), CACHELINE_BYTES,  (nsize+16+MASK_ALLOC_OFFSET(0))*sizeof(float));
        hbw_posix_memalign((void**)(&(next_base)), CACHELINE_BYTES,  (nsize+16+MASK_ALLOC_OFFSET(16))*sizeof(float));
        vel_base  = (float*)_mm_malloc( (nsize+16+MASK_ALLOC_OFFSET(32))*sizeof(float), CACHELINE_BYTES);
#endif
	...
	//
       ...
       initializeFT(p.prev, p.next, p.vel, &p, nbytes, p.n1_Tblock, p.n2_Tblock, p.n3_Tblock, p.num_threads);
